...
class MixinDictRepr(object):
    def __repr__(self, *args, **kwargs):
        return "{class_name} : {content!r} ".format(
            class_name=self.__class__.__name__, content=self.__dict__
        )
...
class MixinMinimalRepr(object):
    ...
    def __repr__(self, *args, **kwargs):
        return "{class_name} : {content!r} ".format(
            class_name=self.__class__.__name__, content=self.__dict__.keys()
        )
...
class BootStrapInfoBox(MixinDictRepr):
    ...
    def current_media_time(self, epoch_timestamp):
        ...
        self._current_media_time = datetime.utcfromtimestamp(
            epoch_timestamp / float(self.time_scale)
        )
...
class FragmentRandomAccessBox(MixinDictRepr):
    ...
    FragmentRandomAccessBoxEntry = namedtuple(
        "FragmentRandomAccessBoxEntry", ["time", "offset"]
    )
    FragmentRandomAccessBoxGlobalEntry = namedtuple(
        "FragmentRandomAccessBoxGlobalEntry",
        ["time", "segment_number", "fragment_number", "afra_offset", "sample_offset"],
    )
...
class SegmentRunTable(MixinDictRepr):
    ...
    SegmentRunTableEntry = namedtuple(
        "SegmentRunTableEntry", ["first_segment", "fragments_per_segment"]
    )
    pass
...
class FragmentRunTable(MixinDictRepr):
    ...
    class FragmentRunTableEntry(
        namedtuple(
            "FragmentRunTableEntry",
            [
                "first_fragment",
        ...
        def __eq__(self, other):
            if (
                self.first_fragment == other.first_fragment
                and self.first_fragment_timestamp == other.first_fragment_timestamp
                and self.fragment_duration == other.fragment_duration
                and self.discontinuity_indicator == other.discontinuity_indicator
            ):
                return True
...
class F4VParser(object):
    ...
    def parse(
        ...
        while bs.pos < bs.len:
            log.debug(
                "Byte pos before header: %d relative to (%d)", bs.bytepos, offset_bytes
            )
            ...
            log.debug(
                "Byte pos after header: %d relative to (%d)", bs.bytepos, offset_bytes
            )
            ...
            if headers_only:
                ...
                parse_function = box_lookup.get(
                    header.box_type, cls._parse_unimplemented
                )
                try:
    ...
    def _read_box_header(bs):
        ...
        return BoxHeader(
            box_size=size - header_size, box_type=box_type, header_size=header_size
        )
    ...
    def _parse_afra(cls, bs, header):
        ...
        for _ in six.range(0, local_entry_count):
            ...
            afra_entry = FragmentRandomAccessBox.FragmentRandomAccessBoxEntry(
    ...
    def _parse_abst(cls, bootstrap_bs, header):
        ...
        (
            ...
            abst.update,
            abst.time_scale,
            abst.current_media_time,
            abst.smpte_timecode_offset,
        ) = box_bs.readlist(
            """pad:8, pad:24, uint:32, uint:2, bool, bool,
                                   pad:4,
